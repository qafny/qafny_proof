...
Proof.
  ...
  rewrite H14.
  rewrite H3. bt_simpl. rewrite H13.
...
Proof.
  ...
  specialize (rz_sub_full (S size) f x B A aenv tenv X1 H1 H7 H11 H12 H6) as eq1.
...
Lemma qft_cu_r_same : forall aenv x c f, nor_mode f c -> fst c <> x -> 0 < aenv x ->
             get_r ((exp_sem aenv (qft_cu x c (aenv x)) f) c) = get_r (f c).
Proof.
  intros. simpl.
  unfold turn_qft.
  ...
  unfold nor_mode, turn_rqft.
  rewrite assign_h_r_lt_same; iner_p.
...
Lemma one_cu_adder_r_same : forall aenv x n c M f, fst c <> x -> n <= aenv x -> 0 < aenv x ->
...
Lemma nor_mode_one_cu_adder : forall x n c M aenv f p, nor_mode f p -> fst p <> x ->
           n <= aenv x -> 0 < aenv x -> nor_mode (exp_sem aenv (one_cu_adder x n c M) f) p.
Proof.
  intros.
  unfold nor_mode in *.
...
Lemma nor_mode_qft_cu : forall x aenv f p, nor_mode f p -> fst p <> x -> 0 < aenv x ->
                nor_mode (exp_sem aenv (qft_cu x p (aenv x)) f) p.
Proof.
  intros.
  unfold nor_mode in *.
  simpl.
...
Lemma nor_mode_qft_acu : forall x aenv f p, nor_mode f p -> fst p <> x -> 0 < aenv x ->
                nor_mode (exp_sem aenv (qft_acu x p (aenv x)) f) p.
Proof.
  intros.
  unfold nor_mode in *.
  simpl.
...
Lemma nor_mode_mod_adder : forall x n c aenv f A M, phi_modes f x n -> n = aenv x ->
...
Lemma phi_modes_qft_cu : forall x n c aenv f, phi_modes f x n -> n = aenv x ->
          nor_mode f c -> fst c <> x -> 0 < aenv x -> phi_modes (exp_sem aenv (qft_cu x c n) f) x n.
Proof. 
  intros.
  unfold phi_modes in *.
  intros. simpl.
  ...
  unfold phi_mode in *.
  bdestruct (i <? aenv x).
  rewrite assign_h_lt_same by lia.
  ...
  rewrite eupdate_index_neq by iner_p.
  unfold turn_rqft.
  rewrite assign_h_r_lt_same; try iner_p.
  rewrite assign_seq_lt ; easy.
  unfold nor_mode.
  unfold turn_rqft.
  rewrite assign_h_r_lt_same; iner_p.
  rewrite assign_seq_lt. easy. lia. lia. 
  unfold nor_mode,turn_rqft.
...
Lemma phi_modes_qft_acu : forall x n c aenv f, phi_modes f x n -> n = aenv x ->
          nor_mode f c -> fst c <> x -> 0 < aenv x -> phi_modes (exp_sem aenv (qft_acu x c n) f) x n.
Proof. 
  intros.
  unfold phi_modes in *.
  intros. simpl.
  ...
  unfold phi_mode in *.
  bdestruct (i <? aenv x).
  rewrite assign_h_lt_same by try lia.
  rewrite assign_r_lt by try lia.
  unfold up_qft.
  rewrite cnot_sem.
  bdestruct (i =? 0). subst.
  ...
  unfold turn_rqft.
  rewrite assign_h_r_lt_same; iner_p.
  rewrite assign_seq_lt ; iner_p. easy. lia. lia.
  unfold nor_mode in *.
  destruct c.
  rewrite eupdate_index_neq by iner_p.
  unfold turn_rqft.
...
Lemma phi_modes_mod_adder : forall x n c aenv f A M, phi_modes f x n ->  nor_mode f c -> fst c <> x 
      -> n = aenv x -> 0 < aenv x -> phi_modes (exp_sem aenv (mod_adder x n c A M) f) x n.
Proof.
  intros.
  unfold mod_adder,mod_adder_half,clean_hbit.
  remember (rz_adder x n A; rz_sub x n M) as e1.
...
Lemma mod_adder_half_well_typed : forall x c size A M aenv tenv,
          ...
          -> well_typed_oexp aenv tenv (mod_adder_half x size c A M) tenv.
Proof.
 intros. unfold mod_adder_half.
 apply pe_seq with (env' := tenv).
 apply pe_seq with (env' := tenv).
 ...
 apply qft_nor. lia. apply Env.add_1. easy.
...
Lemma mod_adder_well_typed : forall x c size A M aenv tenv,
            aenv x = size -> fst c <> x -> Env.MapsTo x (Phi (aenv x)) tenv -> Env.MapsTo (fst c) Nor tenv
          -> well_typed_oexp aenv tenv (mod_adder x size c A M) tenv.
Proof.
 ...
 apply pe_seq with (env' := Env.add x Nor tenv).
 apply pe_seq with (env' := Env.add x Nor tenv).
 apply rqft_phi. lia. rewrite <- H. easy. easy.
 apply cnot_wt_nor; try easy. destruct c. iner_p.
 simpl. apply Env.add_1. easy.
 simpl. apply Env.add_2. iner_p. easy.
 apply qft_nor. lia. apply Env.add_1. easy.
...
Proof.
  ...
  rewrite eq1.
  rewrite fbrev_twice_same. easy. easy.
  apply well_typed_right_mode_pexp with (tenv := tenv).
  apply mod_adder_half_well_typed; try easy. easy.
  apply qft_uniform_exp_trans with (tenv := tenv). easy. easy.
  apply mod_adder_half_well_typed; try easy. easy.
  apply qft_gt_exp_trans with (tenv := tenv). easy.
  apply mod_adder_half_well_typed; try easy. easy.
  apply exp_WF_mod_adder; try lia. easy.
Qed.
...
Proof.
  ...
  assert (exp_sem aenv
     (((rz_adder x (S size) (nat2fb A); rz_sub x (S size) (nat2fb M));
       ((RQFT x (S size); CNOT (@pair var nat x O) c); QFT x (S size)));
      one_cu_adder x (S size) c (nat2fb M)) f
      = (exp_sem aenv
        ( (rz_adder x (S size) (nat2fb A)); rz_compare_half x (S size) c M ;
          QFT x (S size); one_cu_adder x (S size) c (nat2fb M)) f)).
  simpl. easy.
  rewrite H12.
  ...
  bdestruct (B + A <? M).
  rewrite efresh_exp_sem_irrelevant.
  rewrite assign_h_out; try lia.
  rewrite assign_r_out by easy.
  rewrite H15. easy. easy.
  apply exp_WF_rz_adder; try lia.
  apply efresh_rz_adder. easy. lia.
  rewrite assign_h_out; iner_p.
  rewrite assign_r_out by easy.
...
Proof.
  ...
  rewrite one_cu_adder_r_same; try lia.
  rewrite <- H.
  rewrite qft_cu_r_same ; try lia.
  rewrite efresh_exp_sem_irrelevant.
  unfold nor_mode in X1.
  destruct (f c). unfold get_r.
  unfold get_cua in H3. subst.
  unfold put_cu. easy. lia.
  subst. constructor.
  apply exp_WF_rz_adder;try lia.
  apply exp_WF_rz_sub;try lia.
  subst. constructor.
  ...
  subst. constructor.
  apply efresh_rz_adder. easy. lia.
  apply efresh_rz_sub. easy. lia. easy. easy.
  Local Opaque one_cu_adder qft_cu. simpl. Local Transparent one_cu_adder qft_cu.
  apply nor_mode_one_cu_adder; try easy.
  rewrite <- H.
  ...
  constructor.
  apply efresh_rz_adder. easy. lia.
  apply efresh_rz_sub. easy. lia. easy. lia. lia.
  rewrite H13.
  bdestruct (B + A <? M).
  ...
  destruct (f c). subst. easy. lia.
  assert (N.of_nat B < N.of_nat (2^size))%N by lia. simpl in H17.
  rewrite Nofnat_pow in H18. simpl in H18. easy. lia.
...
Lemma phi_nor_mode_rz_modmult' : forall n size y x c aenv f A M, phi_modes f y size -> 
          nor_mode f c -> fst c <> y -> size = aenv y -> 0 < aenv y ->
...
Proof.
 ...
 specialize (IHn size y x c aenv f A M H H0 H1 H2 H3).
 destruct (get_cua
...
Lemma rz_modmult'_x_same: forall n m size x y c A M aenv f, n <= size -> size = aenv y ->
            fst c <> x -> fst c <> y -> x <> y -> snd c < aenv (fst c) ->
...
Proof.
  ...
  Local Transparent mod_adder.
  unfold mod_adder. constructor.
  apply exp_WF_mod_adder;try lia.
  apply exp_WF_clean_hbit; lia.
...
Lemma rz_modmult'_well_typed : forall n size x y c A M aenv tenv, n <= size ->
          ...
          -> well_typed_oexp aenv tenv (rz_modmult' y x n size c A M) tenv.
Proof.
 induction n; intros;simpl.
 constructor. constructor.
 apply pe_seq with (env' := tenv).
...
Proof.
 ...
 specialize (IHn H21 H0 H1 H2 H3 H4
        H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19).
 destruct IHn. 
 rewrite H22. easy. easy. 
 specialize (IHn size y f x c A B M X aenv tenv).
 assert (n <= S size) by lia.
 specialize (IHn H21 H0 H1 H2 H3 H4
        H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19).
 ...
 apply qft_gt_exp_trans with (tenv := tenv); try easy.
 apply rz_modmult'_well_typed; try easy. lia.
 apply exp_WF_rz_modmult'; try lia. iner_p.
 specialize (IHn size y f x c A B M X aenv tenv).
 assert (n <= S size) by lia.
 specialize (IHn H21 H0 H1 H2 H3 H4
        H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19).
...
Proof.
  ...
  assert (exp_sem aenv ((QFT y (S size); rz_modmult' y x (S size) (S size) c A M); RQFT y (S size)) f
    = exp_sem aenv (RQFT y (S size)) 
         (exp_sem aenv (rz_modmult' y x (S size) (S size) c A M)
              (exp_sem aenv (QFT y (S size)) f))).
  simpl. easy.
  rewrite H20. clear H20.
  specialize (rz_modmult'_typed_sem (S size) size y (exp_sem aenv (QFT y (S size)) f)
          x c A B M X aenv (Env.add y (Phi (aenv y)) tenv) H19 H0 H1 H2 H3) as eq1.
  ...
  apply Env.add_2. lia. easy.
  assert (get_cua (exp_sem aenv (QFT y (S size)) f c) = false).
  unfold get_cua.
  rewrite efresh_exp_sem_irrelevant.
  assert (nor_mode f c).
  apply type_nor_mode with (aenv := aenv) (env := tenv); try easy.
  ...
  assert (fbrev (S size) (get_r_qft (exp_sem aenv (QFT y (S size)) f) y) = nat2fb B).
  simpl.
  unfold turn_qft.
  unfold get_r_qft.
  rewrite assign_h_lt_same by lia.
  ...
  unfold up_qft.
  unfold nor_modes,nor_mode in H24.
  assert (0 < S size) by lia.
  specialize (H24 0 H25).
  destruct (f (@pair var nat y O)); try easy.
  assert (fbrev (S size) (get_cus (S size) (exp_sem aenv (QFT y (S size)) f) x) = nat2fb X).
  rewrite get_cus_qft_out. easy.
  constructor. lia. lia.
  easy.
  assert (right_mode_env aenv (Env.add y (Phi (aenv y)) tenv)
        (exp_sem aenv (QFT y(S size)) f) ).
  apply well_typed_right_mode_pexp with (tenv := tenv); try easy.
  apply qft_nor. lia. easy. rewrite H0. easy.
  assert (qft_uniform aenv (Env.add y (Phi (aenv y)) tenv) (exp_sem aenv (QFT y (S size)) f)).
  apply qft_uniform_exp_trans with (tenv := tenv); try easy.
  apply qft_nor. lia. easy. rewrite H0. easy.
  assert (qft_gt aenv (Env.add y (Phi (aenv y)) tenv) (exp_sem aenv (QFT y (S size)) f)).
  apply qft_gt_exp_trans with (tenv := tenv); try easy.
  apply qft_nor. lia. easy. rewrite H0. easy.
  constructor;lia.
  assert (at_match aenv (Env.add y (Phi (aenv y)) tenv)).
  unfold at_match in *.
  intros. bdestruct (x0 =? y). subst.
  apply mapsto_add1 in H30; try lia. inv H30. easy.
  apply H18. apply Env.add_3 in H30. easy. lia.
  specialize (eq1 H20 H21 H22 H23 H8 H9 H10 H11 H12 H25 H26 H27 H28 H29 H30).
  destruct eq1. lia.
  rewrite H31.
  rewrite efresh_exp_sem_irrelevant. easy.
  constructor;lia.
...
Proof.
  ...
  remember ((exp_sem aenv (rz_modmult' y x 
         (S size) (S size) c A M) (exp_sem aenv (QFT y (S size)) f))) as g.
  simpl.
  unfold turn_rqft. rewrite H0.
  rewrite get_cus_assign_hr_same; try lia.
...
Lemma rz_modmult_half_x_same: forall size y c A M aenv f x m,
            fst c <> x -> fst c <> y -> x <> y -> size = aenv y -> snd c < aenv (fst c) ->
...
Proof.
 ...
 unfold times_rotate.
 destruct (exp_sem aenv (rz_adder' x n size M) f (@pair var nat x i)) eqn:eq1. easy. 
...
Proof.
 ...
 apply H4 in H0. unfold phi_mode in H0.
 unfold times_r_rotate.
 destruct (exp_sem aenv (rz_sub' x n size M) f (@pair var nat x i)) eqn:eq1. easy.
 assert ( (@pair Env.key nat x i)  =  (@pair var nat x i)) by easy.
 rewrite H6 in *. rewrite eq1. easy.
...
Proof.
 ...
 apply exp_scope_inv_exp. apply escope_rz_sub.
 rewrite <- H1.
 apply type_phi_modes with (env := tenv) (b:=aenv x); try easy.
 destruct (M n). simpl.
 rewrite IHn with (tenv := tenv); try easy.
 unfold get_r.
 unfold sr_rotate.
 ...
 destruct (f (@pair var nat x i)) eqn:eq1. easy. 
 assert ( (@pair Env.key nat x i)  =  (@pair var nat x i)) by easy.
 ...
 apply well_typed_right_mode_pexp with (tenv := tenv).
 constructor. apply sr_phi with (b := aenv x). easy. lia. easy.
 simpl in H5. easy.
...
Lemma rqft_r : forall i size x aenv tenv f, i < size -> aenv x = size -> 
          Env.MapsTo x (Phi (aenv x)) tenv -> right_mode_env aenv tenv f ->
          get_r (exp_sem aenv (RQFT x size) f (x,i)) = get_r (f (x,i)).
Proof.
  intros; simpl.
  unfold turn_rqft.
  rewrite assign_h_r_lt_same;try lia.
...
Lemma qft_r : forall i size x aenv tenv f, i < size -> aenv x = size -> 
          ...
          get_r (exp_sem aenv (QFT x size) f (x,i)) = get_r (f (x,i)).
Proof.
  intros; simpl.
  unfold turn_qft.
  ...
  unfold get_r.
  assert ( (@pair Env.key nat x i)  =  (@pair var nat x i)) by easy. 
  rewrite H4 in *. rewrite eq1. easy. easy.
Qed.
...
Proof.
  ...
  unfold get_cua.
  destruct b.
  rewrite eupdate_index_neq. easy. easy. easy. easy.
Qed.
...
Lemma qft_cu_r : forall i size x c aenv tenv f, i < size -> aenv x = size ->
      ...
      -> Env.MapsTo (fst c) Nor tenv -> right_mode_env aenv tenv f ->
      get_r (exp_sem aenv (qft_cu x c size) f (x,i)) = get_r (f (x,i)).
Proof.
  intros.
  unfold qft_cu.
  assert ((exp_sem aenv ((RQFT x size; CNOT (@pair var nat x 0) c); QFT x size) f)
    = exp_sem aenv (QFT x size) 
       (exp_sem aenv (CNOT (@pair var nat x 0) c) (exp_sem aenv (RQFT x size) f))).
  simpl. easy.
  rewrite H6.
  ...
  simpl. apply Env.add_1. easy.
  apply well_typed_right_mode_pexp with (tenv := tenv).
  apply rqft_phi. lia. rewrite <- H0. easy. easy. easy.
  apply Env.add_1. easy.
  ...
  apply Env.add_2. iner_p. easy.
  apply well_typed_right_mode_pexp with (tenv := tenv).
  apply rqft_phi. lia. rewrite H0 in *. easy. easy. easy.
Qed.

Lemma qft_acu_r : forall i size x c aenv tenv f, i < size -> aenv x = size ->
      fst c <> x -> snd c < aenv (fst c)-> Env.MapsTo x (Phi (aenv x)) tenv 
      -> Env.MapsTo (fst c) Nor tenv -> right_mode_env aenv tenv f ->
      get_r (exp_sem aenv (qft_acu x c size) f (x,i)) = get_r (f (x,i)).
Proof.
  intros.
  unfold qft_acu.
  assert ((exp_sem aenv ((RQFT x size; ((X (@pair var nat x 0); 
        CNOT (@pair var nat x 0) c); X (@pair var nat x 0))); QFT x size) f)
    = exp_sem aenv (QFT x size) (exp_sem aenv (X (@pair var nat x 0))
       (exp_sem aenv (CNOT (@pair var nat x 0) c)
             (exp_sem aenv (X (@pair var nat x 0)) (exp_sem aenv (RQFT x size) f))))).
  simpl. easy.
  rewrite H6.
  rewrite qft_r with (size := size) (tenv := Env.add x Nor tenv); try easy.
  ...
  apply well_typed_right_mode_pexp with (tenv := tenv).
  apply rqft_phi. lia. rewrite H0 in *. easy. easy. easy. destruct c. iner_p.
  destruct c. iner_p.
  simpl. lia. simpl. apply Env.add_1. easy.
  apply well_typed_right_mode_pexp with (tenv := (Env.add x Nor tenv)).
  constructor. constructor. simpl. apply Env.add_1. easy.
...
Proof.
 ...
 rewrite rz_adder_r with (tenv := tenv); try easy.
  ...
  apply pe_seq with (env' := Env.add x Nor tenv).
  apply pe_seq with (env' := Env.add x Nor tenv).
  apply rqft_phi; try easy. lia. rewrite H0 in *. easy.
...
Proof.
  ...
  assert ( exp_sem aenv ((rz_sub x size M; qft_acu x c size); inv_exp (rz_sub x size M)) f
    = exp_sem aenv (inv_exp (rz_sub x size M)) (exp_sem aenv (qft_acu x c size)
 ...
 apply pe_seq with (env' := Env.add x Nor tenv).
 apply pe_seq with (env' := Env.add x Nor tenv).
 apply rqft_phi. lia. rewrite H0 in *. easy. easy.
 apply pe_seq with (env' := Env.add x Nor tenv).
 apply pe_seq with (env' := Env.add x Nor tenv).
 constructor. constructor. simpl. apply Env.add_1. easy.
 apply cnot_wt_nor. destruct c. iner_p.
...
Proof.
  intros.
  unfold rz_modmult_half in *.
  assert ((exp_sem aenv ((QFT y size; rz_modmult' y x size size c A M); RQFT y size) f)
      = exp_sem aenv (RQFT y size) (exp_sem aenv (rz_modmult' y x size size c A M) (exp_sem aenv (QFT y size) f))).
  simpl. easy.
  ...
  apply Env.add_2. iner_p. easy.
  apply well_typed_right_mode_pexp with (tenv := tenv).
  apply qft_nor. lia. easy. rewrite H1 in *. easy. easy.
  apply Env.add_1. easy.
...
Proof.
  ...
  specialize (X1 n H19) as eq1. unfold nor_mode in eq1.
  unfold put_cu.
  ...
  unfold nor_modes in H20. apply H20 in H19. unfold nor_mode in H19.
  destruct (exp_sem aenv (rz_modmult_half y x (S size) c A M) f (@pair var nat y n)).
  unfold get_cua. unfold get_r in H21. subst. easy. lia. lia.
  rewrite put_cus_neq_2; try lia.
  rewrite efresh_exp_sem_irrelevant; try easy.
  unfold rz_modmult_half.
  constructor. constructor. constructor;lia.
  apply exp_WF_rz_modmult';try lia. easy.
  constructor;lia.
  constructor. constructor.
  constructor. unfold or_not_eq. right. rewrite H0. simpl. lia.
  ...
  bdestruct (v =? x). subst.
  rewrite rz_modmult_half_x_same; try lia. easy. easy. easy. easy.
...
Proof.
  ...
  apply pe_seq with (env' := Env.add x (Phi (aenv x)) tenv). 
  apply qft_nor. lia. easy. rewrite H in *. easy.
  apply rz_modmult'_well_typed; try easy. lia.
  apply Env.add_1. easy.
  apply Env.add_2. lia. easy.
  apply Env.add_2. iner_p. easy.
 ...
 intros.
 bdestruct (k =? x). subst. 
 apply mapsto_add1 in H20. subst. easy.
 apply Env.add_3 in H20. apply Env.add_3 in H20. easy. lia. lia.
  ...
  apply qft_uniform_put_cus_same. easy. easy. easy.
  apply right_mode_exp_put_cus_same. easy. easy.
  ...
  unfold nor_modes; intros.
  nor_mode_simpl. apply X2. easy. easy. lia. simpl. lia.
Qed.
...
Definition nat_full_mult (size:nat) (x y:var) (re:var) :=
   (Rev re ; Rev x); QFT re size;
   (nat_full_mult' size size x y re) ;
  RQFT re size;  (Rev re; Rev x).
...
Definition flt_full_mult (size:nat) (x y:var) (re:var) (ex:var) :=
   (Rev re; Rev x; Rev y); QFT re size;
   (flt_full_mult_quar size x y re ex ; inv_exp (clean_high_flt size size y ex)) ;
  RQFT re size;( (Rev re; Rev x; Rev y)).

(* Implementing x < M comparator. *)
Definition rz_comparator (x:var) (n:nat) (c:posi) (M:nat) := 
   (Rev x); QFT x n;  (rz_sub x n (nat2fb M)); RQFT x n; (CNOT (x,0) c);
  inv_exp ( (Rev x); QFT x n; (rz_sub x n (nat2fb M)); RQFT x n).


(* Implementing x + y / X + M addition. *)
Lemma xfactor_well_typed : forall x aenv tenv,
      Env.MapsTo x Nor tenv -> well_typed_oexp aenv tenv (Rev x; QFT x (aenv x)) (Env.add x (Phi (aenv x)) tenv).
Proof.
 intros.
  apply pe_seq with (env' := tenv).
...
Lemma xfactor_sem : forall x aenv tenv f n, 0 < n -> aenv x = n -> Env.MapsTo x Nor tenv ->
        right_mode_env aenv tenv f -> get_r_qft (exp_sem aenv (Rev x; QFT x n) f) x = fbrev n (get_cus n f x).
Proof.
 intros.
 simpl.
 ...
 assert ((@pair var nat x (Init.Nat.sub (Init.Nat.sub n (S O)) x0)) =
    (@pair Env.key nat x (Init.Nat.sub (Init.Nat.sub n (S O)) x0))) by easy.
 rewrite H8 in *. rewrite eq1 in *. easy. lia. lia. easy.
...
Lemma xfactor_r : forall x aenv tenv f i n, 0 < n -> aenv x = n -> i < n -> Env.MapsTo x Nor tenv ->
        right_mode_env aenv tenv f -> get_r (exp_sem aenv (Rev x; QFT x n) f (x,i)) = get_r (f (x,n-1-i)).
Proof.
 intros.
 simpl.
 assert (nor_modes f x n).
...
Lemma rz_full_adder_sem : forall n size f x y A M aenv tenv, 
           ...
           qft_uniform aenv tenv f -> qft_gt aenv tenv f -> at_match aenv tenv ->
           M < 2^size -> A < 2^size ->
           fbrev size (get_r_qft f x) = nat2fb A ->
...
Proof.
  ...
  assert ((@pair Env.key nat y n) = (@pair var nat y n)) by easy.
  rewrite H13 in *. clear H13. rewrite eq1 in *.
  unfold sr_rotate.
  ...
  assert (right_mode_env aenv tenv (exp_sem aenv (SR (size - S n) x) f)).
  apply well_typed_right_mode_exp; try easy.
  apply sr_phi with (b := aenv x); try easy. rewrite <- H1. lia.
  rewrite bindecomp_seq.
  ...
  apply type_phi_modes with (aenv := aenv) (env := tenv) (b:=aenv x); try easy.
  rewrite <- H1 in H15. unfold phi_modes in H15.
  specialize (H15 0). assert (0 < size) by lia. apply H15 in H16.
  unfold phi_mode in H16.
  unfold times_rotate,rotate.
  destruct (f (x,0)) eqn: eq2.
  assert ((@pair var nat x O) = (@pair Env.key nat x O)) by easy. rewrite H17 in *.
  rewrite eq2 in *. easy.
  assert ((@pair var nat x O) = (@pair Env.key nat x O)) by easy. rewrite H17 in *.
  rewrite eq2 in *.
  unfold qft_uniform in *.
  specialize (H6 x (aenv x) H3 0).
  assert (0 < aenv x) by lia.
  apply H6 in H18. 
  rewrite lshift_fun_0 in *.
  unfold get_snd_r in H18. rewrite eq2 in *.
  rewrite (add_to_sem size); (try easy; try lia).
  rewrite cut_n_fbrev_flip.
  rewrite fbrev_twice_same.
  rewrite H18. rewrite H11.
  rewrite sumfb_correct_carry0.
  rewrite cut_n_mod.
  assert ((size - S (size - S n)) = n) by lia. rewrite H19.
  rewrite <- H12.
  ...
  assert (n <= size) by lia.
  simpl in H14. unfold sr_rotate in H14.
  specialize (IHn size (sr_rotate' f x (S (size - S n)) (S (size - S n))) x y 
      ((A + Nat.b2n (nat2fb M n) * 2 ^ n) mod 2 ^ size) M aenv tenv H16 H0 H1 H2
      H3 H4 H14 ).
  ...
  rewrite bindecomp_seq. 
  rewrite <- H12. rewrite get_cus_cua. rewrite eq1. simpl.
...
Lemma rz_full_adder_well_typed : forall n x y size aenv tenv, x <> y -> n <= size -> size = aenv x -> size = aenv y ->
           Env.MapsTo x (Phi (aenv x)) tenv -> Env.MapsTo y Nor tenv
           -> well_typed_oexp aenv tenv (rz_full_adder' x n size y) tenv.
Proof.
 induction n; intros; simpl.
 constructor. constructor.
 apply pe_seq with (env' := tenv).
 ...
 constructor. constructor. apply sr_phi with (b := aenv x). easy. rewrite <- H1. lia.
 apply IHn; try easy. lia.
Qed.
...
Proof.
  ...
  apply well_typed_right_mode_pexp with (tenv := tenv); try easy.
  constructor. apply sr_phi with (b := aenv x). easy. rewrite <- H0. lia.
  assert ((@pair Env.key nat y n) = (@pair var nat y n)) by easy.
  rewrite H5 in *. clear H5. rewrite eq1 in *.
  apply IHn with (tenv := tenv); try easy. lia.
Qed. 
...
Proof.
 ...
 apply type_phi_mode with (aenv := aenv) (env := tenv) (b := aenv x); try easy. simpl. lia.
 unfold get_r.
 ...
 destruct (f (x,i)) eqn:eq2.
  assert ((@pair Env.key nat x i) = (@pair var nat x i)) by easy.
  rewrite H8 in *. clear H8. rewrite eq2 in *. lia.
  assert ((@pair Env.key nat x i) = (@pair var nat x i)) by easy.
 ...
 easy. simpl. lia. lia.
  replace  (sr_rotate f x (size - S n)) with (exp_sem aenv (SR (size - S n) x) f) by easy.
  apply well_typed_right_mode_pexp with (tenv := tenv); try easy.
  constructor. apply sr_phi with (b := aenv x). easy. rewrite <- H1. lia.
   assert ((@pair Env.key nat y n) = (@pair var nat y n)) by easy.
  rewrite H6 in *. clear H6. rewrite eq1 in *.
 rewrite (IHn size i x y aenv tenv f); try easy. lia.
Qed.

Lemma exp_WF_rz_full_adder: forall n x size y aenv, 
       n <= size <= aenv x -> 0 < aenv x -> aenv x = aenv y -> exp_WF aenv (rz_full_adder' x n size y).
Proof.
  induction n;intros; simpl.
  constructor. simpl. lia.
  constructor.
...
Proof.
  intros.
   ...
   remember (exp_sem aenv (Rev x; QFT x n) f) as g.
   ...
   subst.
   apply qft_gt_exp_trans with (tenv := tenv); try easy.
   constructor. constructor;lia. constructor;lia.
   ...
   rewrite get_put_cus_cut_n; try easy.
   apply f_num_0. destruct H16 as [M eq4]. 
   assert (eq5 := eq4).
  ...
  rewrite H19 in eq4.
  assert (at_match aenv (Env.add x (Phi (aenv x)) tenv)).
  unfold at_match in *.
  intros. bdestruct (x0 =? x). subst.
  apply mapsto_add1 in H20; try lia. inv H20. easy.
  apply H8. apply Env.add_3 in H20. easy. lia.
  specialize (rz_full_adder_sem n n g x y A M aenv (Env.add x (Phi (aenv x)) tenv)
        H18 H0 H1 H2 H16 H17 H9 H10 H11 H20 eq5 eq3 H15 eq4) as eq6.
   ...
   apply rz_full_adder_well_typed; try easy.
   apply exp_WF_rz_full_adder; try lia.
   assert (forall z, z <> x -> (forall i, gf (z,i) = f (z,i))).
   intros. rewrite Heqgf.
   bdestruct (z =? y). rewrite H25 in *.
   ...
   constructor. unfold or_not_eq. left. iner_p.
   rewrite efresh_exp_sem_irrelevant; try easy. rewrite H13. easy. lia.
   apply exp_WF_rz_full_adder; try lia.
   apply efresh_rz_full_adder; try easy.
   apply inv_pexp_reverse with (tenv' := (Env.add x (Phi (aenv x)) tenv)) (tenv := tenv); try easy.
   apply pe_seq with (env' := tenv). constructor. constructor. easy.
   constructor. lia. easy. rewrite <- H1 in *. easy.
   ...
   assert (get_r_qft (exp_sem aenv (Rev x; QFT x n) h) x = fbrev n (get_cus n h x)).
   apply xfactor_sem with (tenv := tenv); try easy.
   rewrite Heqh.
   apply right_mode_exp_put_cus_same; try easy.
   apply functional_extensionality; intros.
   destruct x0.
   bdestruct (v =? x). rewrite H26 in *. clear H26.
   bdestruct (n0 <? n).
   ...
   unfold qft_uniform in H22.
   apply phi_mode_two_same.
   apply type_phi_mode with (aenv := aenv) (env := (Env.add x (Phi (aenv x)) tenv)) (b:= aenv x).
   simpl. apply Env.add_1. easy. simpl in *. rewrite <- H1. lia.
   apply well_typed_right_mode_pexp with (tenv := tenv); try easy.
   apply pe_seq with (env' := tenv).
   constructor. constructor. easy.
   constructor. lia. easy. rewrite H1. easy.
   subst.
   apply right_mode_exp_put_cus_same; try easy.
   apply type_phi_mode with (aenv := aenv) (env := (Env.add x (Phi (aenv x)) tenv)) (b := aenv x).
   simpl. apply Env.add_1. easy. simpl. subst;lia.
   subst.
   apply well_typed_right_mode_pexp with (tenv := (Env.add x (Phi (aenv x)) tenv)); try easy.
   apply rz_full_adder_well_typed; (try easy; try lia).
   rewrite xfactor_r with (tenv := tenv) (n := n); try easy.
   rewrite H28.
   assert ((@pair Env.key nat x n0) = (@pair var nat x n0)) by easy.
   rewrite H29 in *.
   rewrite H27. rewrite Heqg.
   rewrite xfactor_r with (tenv := tenv) (n := n); try easy.
   subst.
   apply right_mode_exp_put_cus_same; try easy.
   rewrite H22 with (b := n); try easy. rewrite eq6.
   assert (qft_uniform aenv (Env.add x (Phi (aenv x)) tenv) (exp_sem aenv (Rev x; QFT x n) h) ).
   rewrite Heqh.
   apply qft_uniform_exp_trans with (tenv := tenv); try easy.
   apply qft_uniform_put_cus_same; try easy.
   apply pe_seq with (env' := tenv).
   ...
   constructor. lia. easy. rewrite H1. easy.
   apply right_mode_exp_put_cus_same; try easy.
   unfold qft_uniform in H29. rewrite H29 with (b := (aenv x)). rewrite H25. rewrite Heqh.
   assert ((get_cus n f x) = cut_n (get_cus n f x) n).
   remember (cut_n (get_cus n f x) n) as t.
   rewrite <- put_get_cus_eq with (f := f) (x := x) (n := n); try easy.
   rewrite get_put_cus_cut_n; try easy.
   rewrite <- H30 in *. rewrite eq2. rewrite H19. rewrite eq4.
   rewrite sumfb_correct_carry0.
   rewrite bindecomp_spec.
   ...
   rewrite H1.
   apply type_nor_modes with (aenv := aenv) (env := tenv); try easy. easy. lia.
   rewrite H1.
   apply Env.add_1. easy.
   ...
   rewrite put_cus_neq_2; try lia.
   rewrite efresh_exp_sem_irrelevant with (p := (x,n0)).
   rewrite efresh_exp_sem_irrelevant with (p := (x,n0)). easy.
   constructor. constructor. lia. constructor;lia.
   constructor. constructor. unfold or_not_eq. right. simpl in *. lia.
   constructor. unfold or_not_eq. right. simpl in *. lia.
   apply exp_WF_rz_full_adder;try lia.
   apply exp_fresh_rz_full_adder'_ge; try lia.
   constructor. constructor. lia. constructor;lia.
   constructor. constructor. unfold or_not_eq. right. simpl in *. lia.
   constructor. unfold or_not_eq. right. simpl in *. lia.
   subst.
   rewrite efresh_exp_sem_irrelevant with (p := (v,n0)).
   rewrite put_cus_neq; try lia.
   bdestruct (v =? y). subst.
   rewrite rz_full_adder_y_same with (tenv := (Env.add x (Phi (aenv x)) tenv)); try easy.
   rewrite efresh_exp_sem_irrelevant with (p := (y,n0)). easy.
   constructor. constructor. lia. constructor;lia.
   constructor. constructor. unfold or_not_eq. left. simpl in *. lia.
   constructor. unfold or_not_eq. left. simpl in *. lia.
   rewrite efresh_exp_sem_irrelevant with (p := (v,n0)).
   rewrite efresh_exp_sem_irrelevant with (p := (v,n0)). easy.
   constructor. constructor. lia. constructor;lia.
   constructor. constructor. unfold or_not_eq. left. simpl in *. lia.
   constructor. unfold or_not_eq. left. simpl in *. lia.
   apply exp_WF_rz_full_adder;try lia.
   apply efresh_rz_full_adder; try lia. iner_p. iner_p.
   constructor. constructor. lia. constructor;lia.
   constructor. constructor. unfold or_not_eq. left. simpl in *. lia.
   constructor. unfold or_not_eq. left. simpl in *. lia.
Qed.
...
Definition rz_adder_form (x:var) (n:nat) (M:nat -> bool) :=
   (Rev x); QFT x n; rz_adder x n M ;
  inv_exp ( (Rev x); QFT x n).

Lemma rz_adder_form_well_typed : forall n x M aenv tenv, aenv x = n ->
      Env.MapsTo x Nor tenv -> well_typed_oexp aenv tenv (rz_adder_form x n M) tenv.
Proof.
 intros. unfold rz_adder_form.
  ...
  apply pe_seq with (env' := tenv).
  constructor. constructor. easy.
  constructor; try easy. lia. rewrite H. easy.
  Local Transparent rz_adder.
  unfold rz_adder.
  Local Opaque rz_adder.
...
Proof.
  intros.
   ...
   apply type_nor_modes with (aenv := aenv) (env := tenv); try easy.
   assert (fbrev n (get_r_qft g x) = get_cus n f x).
   rewrite H10. rewrite fbrev_twice_same. easy.
   clear H10.
   assert (exists A, (get_cus n f x) = nat2fb A).
   rewrite <- put_get_cus_eq with (f := f) (x := x) (n := n); try easy.
   rewrite get_put_cus_cut_n; try easy.
   apply f_num_0. destruct H10 as [A eq2]. rewrite eq2 in H13.
   assert (eq3 := eq2).
   rewrite <- put_get_cus_eq with (f := f) (x := x) (n := n) in eq2; try easy.
   rewrite get_put_cus_cut_n in eq2; try easy.
   apply f_num_small in eq3.
  ...
  unfold at_match in *.
  intros. bdestruct (x0 =? x). subst.
  apply mapsto_add1 in H15; try lia. inv H15. easy.
  apply H6. apply Env.add_3 in H15. easy. lia. 
   apply rz_adder_well_typed; try easy.
   assert (qft_gt aenv (Env.add x (Phi (aenv x)) tenv) gf).
   subst.
   apply qft_gt_exp_trans with  (tenv := (Env.add x (Phi (aenv x)) tenv)); try easy.
   ...
   assert (forall z, z <> x -> (forall i, gf (z,i) = f (z,i))).
   intros. rewrite Heqgf.
   rewrite efresh_exp_sem_irrelevant; try easy. rewrite H11. easy. lia.
   apply exp_WF_rz_adder;try lia.
   apply efresh_rz_adder; try easy.
   apply inv_pexp_reverse with (tenv' := (Env.add x (Phi (aenv x)) tenv)) (tenv := tenv); try easy.
   apply pe_seq with (env' := tenv). constructor. constructor. easy. constructor. lia. easy.
   rewrite H1. easy.
   apply right_mode_exp_put_cus_same; try easy.
   apply qft_uniform_put_cus_same; try easy.
   apply qft_gt_put_cus_same; try easy.
   ...
   apply functional_extensionality; intros.
   destruct x0.
   bdestruct (v =? x). rewrite H19 in *. clear H19.
   bdestruct (n0 <? n).
   assert (get_r (gf (x,n0)) = get_r (g (x,n0))).
   rewrite Heqgf. 
   ...
   assert ((@pair Env.key nat x n0) = (@pair var nat x n0)) by easy.
   rewrite H22 in *.
   rewrite H20. rewrite Heqg.
   rewrite xfactor_r with (tenv := tenv) (n := n); try easy.
   subst.
   apply right_mode_exp_put_cus_same; try easy.
   rewrite H15 with (b := aenv x); try easy. rewrite eq4.
   assert (qft_uniform aenv (Env.add x (Phi (aenv x)) tenv) (exp_sem aenv (Rev x; QFT x n) h) ).
   rewrite Heqh.
   apply qft_uniform_exp_trans with (tenv := tenv); try easy.
   apply qft_uniform_put_cus_same; try easy.
   ...
   rewrite H1. easy.
   apply right_mode_exp_put_cus_same; try easy.
   unfold qft_uniform in H21. rewrite H22. rewrite H18. rewrite Heqh.
   assert ((get_cus n f x) = cut_n (get_cus n f x) n).
   remember (cut_n (get_cus n f x) n) as t.
   rewrite <- put_get_cus_eq with (f := f) (x := x) (n := n); try easy.
   rewrite get_put_cus_cut_n; try easy. rewrite H23. rewrite eq2.
   rewrite get_put_cus_cut_n; try easy.
   rewrite sumfb_correct_carry0.
   rewrite cut_n_mod.
   rewrite plus_comm. easy.
...
Proof.
  ...
  rewrite H7.
  apply f_num_small in H7 as eq1.
  rewrite rz_adder_form_correct with (tenv := tenv); try easy.
  rewrite <- H7. rewrite <- put_cus_sumfb_cut. easy.
Qed.
...
Definition rz_full_sub_form (x:var) (n:nat) (y:var) :=
   (Rev x; QFT x n) ; rz_full_sub x n y ; 
  inv_exp (Rev x; QFT x n).
...
Proof.
  ...
  apply well_typed_right_mode_exp; try easy.
  apply srr_phi with (b := aenv x); try easy. rewrite <- H1. lia.
  rewrite bindecomp_seq.
  ...
  rewrite nat2fb_bound with (n := size) by lia. easy.
  assert (n <= size) by lia.
  simpl in H14. unfold srr_rotate in H14.
  specialize (IHn size (srr_rotate' f x (S (size - S n)) (S (size - S n))) x y 
      ((A + 2 ^ size - Nat.b2n (nat2fb M n) * 2 ^ n) mod 2 ^ size) M aenv tenv H16 H0 H1 H2
      H3 H4 H14 ).
  rewrite IHn ; try easy.
...
Lemma rz_full_sub_well_typed : forall n x y size aenv tenv, x <> y -> n <= size -> size = aenv x -> size = aenv y ->
           Env.MapsTo x (Phi (aenv x)) tenv -> Env.MapsTo y Nor tenv
           -> well_typed_oexp aenv tenv (rz_full_sub' x n size y) tenv.
Proof.
 induction n; intros; simpl.
 constructor. constructor.
 apply pe_seq with (env' := tenv).
  apply pcu_nor; try easy. simpl.
 constructor. unfold or_not_r. left. simpl. lia.
 constructor. constructor. apply srr_phi with (b := aenv x). easy. rewrite <- H1. lia.
 apply IHn; try easy. lia.
Qed.
...
Proof.
  ...
  apply well_typed_right_mode_pexp with (tenv := tenv); try easy.
  constructor. apply srr_phi with (b := aenv x). easy. rewrite <- H0. lia.
  assert ((@pair Env.key nat y n) = (@pair var nat y n)) by easy.
  rewrite H5 in *. clear H5. rewrite eq1 in *.
  apply IHn with (tenv := tenv); try easy. lia.
Qed. 
...
Proof.
 ...
 easy. simpl. lia. lia.
  replace  (srr_rotate f x (size - S n)) with (exp_sem aenv (SRR (size - S n) x) f) by easy.
  apply well_typed_right_mode_pexp with (tenv := tenv); try easy.
  constructor. apply srr_phi with (b := aenv x). easy. rewrite <- H1. lia.
   assert ((@pair Env.key nat y n) = (@pair var nat y n)) by easy.
  rewrite H6 in *. clear H6. rewrite eq1 in *.
 rewrite (IHn size i x y aenv tenv f); try easy. lia.
Qed.

Lemma exp_WF_rz_full_sub: forall n x size y aenv, 
       n <= size <= aenv x -> 0 < aenv x -> aenv x = aenv y -> exp_WF aenv (rz_full_sub' x n size y).
Proof.
  induction n;intros; simpl.
  constructor. simpl. lia.
  constructor.
...
Proof.
  ...
  specialize (rz_full_sub_sem n n g x y A M aenv (Env.add x (Phi (aenv x)) tenv)
   ...
   apply qft_gt_exp_trans with  (tenv := (Env.add x (Phi (aenv x)) tenv)); try easy.
   apply rz_full_sub_well_typed; try easy.
   apply exp_WF_rz_full_sub; try lia.
   assert (forall z, z <> x -> (forall i, gf (z,i) = f (z,i))).
   intros. rewrite Heqgf.
   bdestruct (z =? y). rewrite H25 in *.
   ...
   unfold qft_uniform in H22.
   apply phi_mode_two_same.
   apply type_phi_mode with (aenv := aenv) (env := (Env.add x (Phi (aenv x)) tenv)) (b := aenv x); try easy.
   simpl. lia.
   apply well_typed_right_mode_pexp with (tenv := tenv); try easy.
   apply pe_seq with (env' := tenv). constructor. constructor. easy. constructor. lia. easy.
   rewrite H1. easy.
   ...
   subst.
   apply right_mode_exp_put_cus_same; try easy.
   rewrite H22 with (b := aenv x); try easy. rewrite eq6.
   assert (qft_uniform aenv (Env.add x (Phi (aenv x)) tenv) (exp_sem aenv (Rev x; QFT x n) h) ).
   rewrite Heqh.
   apply qft_uniform_exp_trans with (tenv := tenv); try easy.
   apply qft_uniform_put_cus_same; try easy.
   ...
   rewrite <- put_get_cus_eq with (f := f) (x := x) (n := n); try easy.
   rewrite get_put_cus_cut_n; try easy.
   rewrite <- H30 in *. rewrite eq2. rewrite eq4.
   ...
   rewrite <- H1. easy.
   simpl.
   unfold turn_qft. rewrite H26 in *.
   rewrite assign_h_ge; try lia.
   rewrite assign_r_ge by lia.
   unfold reverse. simpl.
   rewrite <- H1. bdestruct (n0 <? n). lia. bdestruct ((x =? x)). simpl.
   ...
   easy. lia.
   apply exp_WF_rz_full_sub; try lia.
   apply exp_fresh_rz_full_sub'_ge; try lia.
   lia. rewrite H24.
   simpl.
   unfold turn_qft.
   rewrite assign_h_out; iner_p.
...
Definition rz_sub_right (x:var) (n:nat) (M:nat -> bool) :=
   (Rev x; QFT x n); rz_sub x n M ; inv_exp (Rev x; QFT x n).
...
Proof.
  ...
  apply H6. apply Env.add_3 in H15. easy. lia.
   ...
   rewrite H1. easy.
   apply right_mode_exp_put_cus_same; try easy.
   unfold qft_uniform in H22. rewrite H22 with (b := aenv x); try easy. rewrite H18. rewrite Heqh.
   assert ((get_cus n f x) = cut_n (get_cus n f x) n).
   remember (cut_n (get_cus n f x) n) as t.
   rewrite <- put_get_cus_eq with (f := f) (x := x) (n := n); try easy.
   rewrite get_put_cus_cut_n; try easy. rewrite H23. rewrite eq2.
   ...
   rewrite assign_r_ge by lia.
   unfold reverse. simpl.
   bdestruct (n0 <? aenv x). lia. bdestruct ((x =? x)). simpl.
   rewrite put_cus_out by lia.
   rewrite efresh_exp_sem_irrelevant with (p := (x,n0)).
   simpl.
   rewrite assign_h_ge by lia.
...
Proof.
  ...
  rewrite <- H7. rewrite <- put_cus_sumfb_neg_cut. easy.
Qed.


(* Implementing M - x subtractor. *)
Definition rz_sub_left (M:nat -> bool) (x:var) (n:nat) :=
   (Rev x); QFT x n; rz_sub x n M; inv_exp ( (Rev x); QFT x n); negator0 n x.

(* Implementing x < y comparator. *)
Definition rz_full_comparator (x:var) (n:nat) (c:posi) (y:var) := 
     (Rev x; Rev y); QFT x n; QFT y n; (rz_full_sub x n y); RQFT x n;  (CNOT (x,0) c);
    inv_exp ( (Rev x; Rev y); QFT x n; QFT y n;  (rz_full_sub x n y); RQFT x n).
...
Definition rz_compare_half2 (x:var) (n:nat) (c:posi) (M:nat -> bool) := 
   (rz_sub x n M) ; RQFT x n;  (X (x,0); CNOT (x,0) c ; X (x,0)).
...
Definition appx_compare_half3 (x:var) (n:nat) (b:nat) (c:posi) (M:nat -> bool) := 
   (appx_sub x n b M) ; RQFT x b ; (SWAP (x,0) c).
...
Fixpoint rshift_by_swap (n:nat) (x:var) :=
    match n with 0 => SKIP (x,0)
              | S m => rshift_by_swap m x; SWAP (x,m) (x,n)
    end.

Fixpoint lshift_by_swap (n:nat) (x:var) :=
    match n with 0 => SKIP (x,0)
...
Fixpoint appx_moder'_a i (n:nat) (b:nat) (x ex:var) (M:nat -> bool) := 
     match i with 0 =>  (SKIP (x,0))
           | S j => appx_compare_half3 x n b (ex,j) M ;  rshift_by_swap n x;
                     QFT x (b-1); (CU (ex,j) ((appx_adder x n (b-1) M)));
                      (X (ex,j)); 
                       appx_moder'_a j n (b-1) x ex (cut_n (div_two_spec M) n)
     end.
...
Definition vars_for_app_div_mod (size:nat) := 
  gen_vars (S size) (x_var::(y_var::(([])))).

Definition avs_for_app_div_mod (size:nat) := fun x => (x/ (S size), x mod (S size)).

Definition trans_app_div_mod (size M:nat) :=
  trans_exp (vars_for_app_div_mod size) (2 * (S size)) 
     (app_div_mod_out size M) (avs_for_app_div_mod size). 

Definition app_div_mod_aout (size:nat) := 
   appx_div_mod_a (S size) x_var y_var.
